...
from xhs.help import (cookie_jar_to_cookie_str, download_file,
...
class XhsClient:
    ...
    def post(self, uri: str, data: dict, is_creator: bool = False, **kwargs):
        ...
        # print("JSON",json_str.encode())
        # print("url", f"{self._creator_host if is_creator else self._host}{uri}")
        return self.request(
    ...
    def get_note_by_id(self, note_id: str ,xsec_token:str):
        ...
        :rtype: dict
        """
        data = {"source_note_id": note_id, "image_scenes": ["CRD_WM_WEBP"],"xsec_source":"pc_feed","xsec_token":xsec_token}
    ...
    def get_note_by_id_from_html(self, note_id: str,xsec_token:str):
        ...
        url = "https://www.xiaohongshu.com/explore/" + note_id+"?xsec_token=" + xsec_token+"&xsec_source=pc_cfeed"
        # url="https://www.xiaohongshu.com/explore/6764b5850000000013003ecb?xsec_token=ABADDmRc2-jDXYs6sOmgP8XEnX_AkN9aRzjpm8mqTrdAE=&xsec_source=pc_cfeed"
        res = self.session.get(url, headers={"user-agent": self.user_agent, "referer": "https://www.xiaohongshu.com/"},proxies=self.proxies)
        html = res.text
        try:
            state = re.findall(r"window.__INITIAL_STATE__=({.*})</script>", html)[0].replace("undefined", '""')
            if state != "{}":
                note_dict = transform_json_keys(state)
                return note_dict["note"]['note_detail_map'][note_id]
    ...
    def get_home_feed(self, feed_type: FeedType,search_key):
        ...
        data = {
            ...
            "refresh_type": 2,
            "note_index": 1,
            "unread_begin_note_id": "67657936000000001402655a",
            "unread_end_note_id": "",
            "unread_note_count": 100,
    ...
    def get_note_all_comments(self, note_id: str,xsec_token:str, crawl_interval: int = 1):
        ...
        comments_cursor = ""
        num=1
        while comments_has_more and num<30:
            comments_res = self.get_note_comments(note_id=note_id, cursor=comments_cursor,xsec_token=xsec_token)
            ...
            for comment in comments:
                ...
                #         note_id, comment["id"], num=page_num, cursor=sub_comment_cursor,xsec_token=xsec_token
                #     )
